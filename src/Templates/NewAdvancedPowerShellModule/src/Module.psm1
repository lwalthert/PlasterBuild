...
. (Join-Path $ModulePath 'other\PreLoad.ps1') @ProfilePathArg

# Private and other methods and variables
Get-ChildItem (Join-Path $ModulePath 'private') -Recurse -Filter "*.ps1" -File | Sort-Object Name | ForEach-Object {
    Write-Verbose "Dot sourcing private script file: $($_.Name)"
    . $_.FullName
}

# Load and export public methods
Get-ChildItem (Join-Path $ModulePath 'public') -Recurse -Filter "*.ps1" -File | Sort-Object Name | ForEach-Object {
...
. (Join-Path $ModulePath 'other\PostLoad.ps1')